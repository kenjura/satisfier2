// extractRecipes.mjs
// Usage: node extractRecipes.mjs
// Reads en-US.json and outputs recipes in recipes.js format (to stdout)

import fs from 'fs';
import path from 'path';
import altScores from './altScores.mjs';
import rawData from './raw-data-20250917.json' assert { type: 'json' };
  ...
  for (const [className, entry] of classMap.entries()) {
    ...
    if (products.length > 0) {
      const rec = {
        ...
        alternate: entry.mDisplayName.match('Alternate') ? 'yes' : 'no',
